...
- [ ] `.env` da API: ao menos `DATABASE_URL`, `SESSION_SECRET`, `SEED_*`, e **`APP_URL=http://localhost:5173`**. (Anthropic = Codi; Resend = e-mails; Google = Classroom — todos opcionais p/ a maior parte do teste.)
- [ ] `docker compose up -d` · `pnpm --filter @codinhos/api db:migrate` · `pnpm --filter @codinhos/api db:seed`
- [ ] Subir dev: `pnpm dev:local` (sem Turbo). API em `:3333`, SPA em `:5173`.
- [ ] Abrir `http://localhost:3333/health` → responde OK.
...
- [ ] Acessar `/__system__/login` e logar com `SEED_SUPER_ADMIN_EMAIL` / senha do `.env`. → cai na área **/admin**.
- [ ] **Escolas (Tenants)**: ver `Escola Demo` na lista; criar uma escola nova (nome + slug). → aparece na lista.
- [ ] **Badges**: ver/criar uma badge (slug, nome, gatilho, valor). → salva e lista.
- [ ] **Usuários**: listagem global com filtro por papel. → filtra corretamente.
- [ ] Logout (canto do menu). → volta pro login.

---

## B. Gestor — configuração da escola

- [ ] Logar em `/escola-demo/login` com `gestor@escola-demo.com`. → cai em **/manager** (Dashboard).
- [ ] **Dashboard**: ver KPIs (Alunos, Ativos hoje, Turmas) e seção de **Alertas**. → KPIs clicáveis levam a Alunos/Turmas.
- [ ] **Turmas** → criar turma: nome + **modo de progressão** (livre/sequencial/controlado) + **validação** (auto / auto+revisão / manual). → turma criada com os badges de modo.
- [ ] Abrir o **detalhe da turma** → atribuir uma **trilha** à turma. → trilha listada.
- [ ] **(p/ testar blocos)** Em uma trilha da turma, ligar o `visualBlocksEnabled`. → toggle salvo.
- [ ] **Alunos** → adicionar um aluno existente à turma. → aparece na lista de alunos da turma.
- [ ] **Importação CSV**: baixar o template, preencher 2 linhas (`nome,email`), importar. → mostra `criados / ignorados / erros`; e-mail repetido conta como ignorado.
- [ ] **Criar professor**: em Usuários, criar um usuário com papel **professor**. → criado; (recebe convite).
- [ ] **Vincular professor à turma** (aba Professores da turma). → 422 se o usuário não for professor; 409 se repetir; sucesso lista o professor.
- [ ] **Criar responsável** vinculando 1+ alunos. → 409 e-mail repetido; 404 se algum aluno inválido (e o responsável **não** é criado); sucesso lista o responsável com a contagem de filhos.
- [ ] **Configurações** → alternar `aiErrorExplanationEnabled`. → salva (reflete no aluno, passo C).
- [ ] **(Opcional, se Google configurado)** Config → "Conectar Google" → consentimento → volta com aviso de conectado → listar cursos → importar um curso. → cria turma com os alunos do Classroom.

---

## C. Aluno — aprender, gamificar, certificar

- [ ] Logar com `aluno@escola-demo.com`. → cai em **/learn** (Trilhas).
- [ ] Abrir uma **trilha** → um **módulo** → o **desafio**. → vê conceito, exemplo e o editor.
- [ ] **Autocomplete contextual**: começar a digitar no editor. → sugere **só** o vocabulário ensinado até esse módulo (não o JS inteiro).
- [ ] **Submeter** a solução correta. → testes passam (verde), painel mostra **XP ganho**, possível **subida de nível** e **badge**.
- [ ] Submeter um código com **erro** (ex.: variável não definida). → mensagem **humanizada** (sem stack trace).
- [ ] **(Se Codi/Anthropic ligado)** clicar "Pedir ajuda ao Codi" sobre um teste que falhou. → resposta contextual; respeita limite diário. (Se o toggle do gestor estiver off, o botão **não** aparece.)
- [ ] **Sino de notificações** → ver badge/level-up/streak. → lista as notificações.
