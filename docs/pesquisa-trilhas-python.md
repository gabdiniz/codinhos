...
| G2 | ~~`input()` sem fonte de stdin simulável~~ **RESOLVIDO (12/07/2026)** | O modelo do motor é entrada→saída via argumento de função, não leitura interativa de stdin; nem JS tem `prompt()` testável hoje (gap não resolvido pro lado JS, nenhum desafio JS precisa) | **M** — implementado: `TestCase.stdin?: string[]` (fila consumida em ordem por `input()`), `input` redefinido no dict de globals isolado da execução (não muta `builtins`, preserva o isolamento entre submissões), fila esgotada dá `EOFError` igual o Python real. Ver `docs/motor-python-capacidades.md` §1.4 | Trilha 1 pode passar a EXIGIR `input()` de verdade em algum desafio (não migrada nesta rodada — só o motor foi mexido) |
| G3 | ~~Distinção tupla vs. lista se perde na serialização JSON~~ **RESOLVIDO (12/07/2026)** | `deepEqual`/JSON não preserva o tipo Python de coleção — resolvido "de graça" na P1 pra comparação padrão (tupla e lista comparam iguais, por design); versão completa agora existe como OPÇÃO | **P** — implementado: `TestCase.expectedType?: string` (`'tuple'`, `'list'`, etc.), checado em cima do `actualType` que o worker já calculava desde a P1 (`type(result).__name__`), só em function-call/instance-call. Ver `docs/motor-python-capacidades.md` §1.5 | Trilha 5 (mitigado desde a P1: nenhum desafio exige isso na nota; agora é possível se algum desafio futuro quiser exigir) |
| G4 | ~~Sem modo de teste para exceções (`try/except`)~~ **RESOLVIDO (12/07/2026)** | Motor não tinha conceito de "espera-se que lance erro X" | **M** — implementado: `mode: 'raises'`, reusa `expected` como nome da classe da exceção (comparação por nome exato, reconhece exceções customizadas do aluno também). `python-exec.ts` ganhou `errorClassName` no `RunResponse` (reaproveitando a captura de exceção que já existia desde a P1). Ver `docs/motor-python-capacidades.md` §1.6 | Nenhuma das 10 usa isso ainda (currículo de tratamento de erro continua fora do currículo por decisão de PRODUTO, não mais por limitação de motor — a decisão de trazer isso pro conteúdo é separada) |
| G5 | ~~Verificação estrutural (`mode:'ast'`) para Python precisa de implementação própria~~ **RESOLVIDO (12/07/2026)** | O heurístico de JS não serve para sintaxe Python (indentação, `:` em vez de `{}`); a biblioteca `ast` nativa do Python resolveu isso melhor do que o heurístico de JS, e sem executar o código do aluno (só `ast.parse`) | **M** — implementado: `mode:'ast'` em `packages/runner-python`, os 6 `AstRuleKind` existentes via `ast.walk`, fechando de brinde o loophole de comprehension-como-loop. Ver `docs/motor-python-capacidades.md` §1.2 | Trilha 8 pode migrar pra `requireRecursion`/`forbidLoops` de verdade (não migrada nesta rodada — só o motor foi mexido) |
| G6 | ~~Escopo de módulos padrão liberados não está definido~~ **RESOLVIDO (12/07/2026)** | Motor JS cura globais (`SAFE_GLOBALS`); Python precisava da mesma curadoria para `import` | **P** — implementado: allowlist fechada (`math`/`random`/`string`/`functools`) checada via `ast.parse` (nunca executa o código) antes de qualquer execução real, em `packages/runner-python`. `functools` entrou na lista por causa da trilha 7 (`reduce`), achado ao revisar o seed já escrito — não estava nos "candidatos óbvios" originais desta linha. Ver `docs/motor-python-capacidades.md` §1.3 | Trilha 10 (e trilha 7, `functools`) |
