...
| G7 | **POO exercita só via stdout, não há modo "instancie e chame método"** | O motor pega "a primeira função declarada"; uma classe não é uma função — então testar estado de objeto exige o aluno **imprimir** o resultado (`print`), não `return` de instância | **M** (se quiser um modo `mode:'instance-call'` dedicado; não bloqueia a trilha, só limita o tipo de desafio de POO) | Trilha 9 (contornado no desenho, ver doc da trilha) |
