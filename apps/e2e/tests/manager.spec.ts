...
test.describe('Dashboard do gestor', () => {
  test('deve exibir KPIs principais', async ({ managerPage: page }) => {
    ...
    await expect(page.locator('[class*="statCard"]').first()).toBeVisible()
  })
...
test.describe('ClassesPage — turmas', () => {
  ...
  test('deve listar turmas ou exibir estado vazio', async ({ managerPage: page }) => {
    const hasClasses = await page.locator('[class*="tableRow"]').first().isVisible()
    const isEmpty    = await page.getByText(/nenhuma turma/i).isVisible()
    expect(hasClasses || isEmpty).toBe(true)
  })
...
test.describe('StudentsPage — alunos', () => {
  ...
  test('deve listar alunos ou exibir estado vazio', async ({ managerPage: page }) => {
    const hasStudents = await page.locator('[class*="tableRow"]').first().isVisible()
    const isEmpty     = await page.getByText(/nenhum aluno/i).isVisible()
    expect(hasStudents || isEmpty).toBe(true)
  })
  ...
  test('deve filtrar alunos por status usando tabs', async ({ managerPage: page }) => {
    ...
    const tabAtivos = page.getByRole('tab', { name: 'Ativos' })
    await expect(tabAtivos).toBeVisible()
    await tabAtivos.click()
    // Página não quebra após filtrar
    await expect(page.locator('[class*="root"]')).toBeVisible()
  })

  test('deve buscar aluno por nome via campo de busca', async ({ managerPage: page }) => {
    const search = page.getByRole('textbox', { name: 'Buscar alunos' })
    await expect(search).toBeVisible()
    await search.fill('Ana')
    // Lista deve atualizar sem quebrar
    await expect(page.locator('[class*="root"]')).toBeVisible()
  })
})
