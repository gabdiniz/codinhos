...
export default function ChildDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null)
    try {
      const res = await api.get<{ data: ChildDetail }>(
        `/api/${slug}/guardian/children/${studentId}`,
      )
      setData(res.data)
    } catch (err) {
  ...
  if (loadError || !data) {
    return <div className={styles.state}><span className={styles.stateError}>{loadError ?? 'Não encontrado.'}</span></div>
  }
  ...
  return (
    <div className={styles.root}>
      <Link to={`/${slug}/guardian`} className={styles.backLink}>
        <IconArrowLeft /> Filhos
      </Link>
      ...
      <header className={styles.pageHeader}>
        ...
        <div>
          ...
          <div className={styles.headerStats}>
            <strong className={styles.level}>Nível {stats.level}</strong>
