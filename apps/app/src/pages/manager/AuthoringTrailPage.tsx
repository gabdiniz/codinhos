      ...
      <form className={`${styles.modal} ${styles.modalWide}`} onClick={(e) => e.stopPropagation()} onSubmit={submit}>
        ...
        {rows.map((r, i) => (
          <div key={i} className={styles.testCase}>
            ...
            {r.mode === 'ast' && (
              <small className={styles.hint}>Verifica a <b>estrutura</b> do código (não usa input/esperado) — ex.: exigir recursão ou proibir laços. Combine com casos normais para checar também o resultado. Use a <b>descrição</b> para explicar a regra ao aluno.</small>
            )}
          </div>
        ))}
